...
test.describe('to do page tests', () => {
    ...
    test('add test', async ({ page }) => {
        ...
        await expect(page.locator("[class='Form']").first()).toHaveText(/todo test/);
    });

    test('edit test', async ({ page }) => {
        let target = page.locator('[class="Form"]:has-text("todo test")').locator('text=edit');
        ...
        await expect(page.locator("[class='Form']").first()).toHaveText(/test todo/);
    });
